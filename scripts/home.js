...
async function renderHomeRiskSummary(){
  ...
  if(!el)return;
  const loadingLabels=['오늘 마감','지연 중','내 이슈','미청구','오늘 휴가'];
  ...
  try{
    ...
    const todayDueProjects=(projects||[]).filter(project=>{
      ...
      if(!endDateRaw||isCompletedProject(project))return false;
    ...
    const pendingDocRows=(pendingDocs||[]).filter(row=>{
      ...
      return toDate(row.due_date)<=dueLimit;
    }).sort((a,b)=>{
    ...
    const getTodayScheduleMemberNames=type=>[...new Set((schedules||[])
      .filter(schedule=>{
        ...
        if(scheduleType!==type)return false;
    ...
    const availableMemberCount=Math.max(activeMemberCount-todayLeaveCount,0);
    ...
    const cards=[
      {
        ...
        emphasis:'action',
        meta:todayDueProjects.length?('프로젝트 관리에서 '+todayDueProjects.length+'건 확인'):'오늘 마감 프로젝트 없음',
      ...
      },
      {
        ...
        emphasis:'action',
        meta:oldestOverdue?('프로젝트 관리에서 D+'+oldestOverdueDiff+' 확인'):'지연 프로젝트 없음',
        action:"setPage('projects')"
      },
      {
        label:'내 이슈',
        value:myOpenIssues.length?myOpenIssues.length+'건':'없음',
        tone:myOpenIssues.length?'warning':'neutral',
        quiet:!myOpenIssues.length,
        emphasis:'action',
        meta:myOpenIssues.length
          ?(myHighPriorityIssues.length?('Issues에서 우선 확인 '+myHighPriorityIssues.length+'건'):'Issues에서 '+myOpenIssues.length+'건 확인')
          :(currentMember?.name?'내 담당 열린 이슈 없음':'담당자 정보 없음'),
      ...
      {
        label:'오늘 휴가',
        value:activeMemberCount?(todayLeaveCount+'/'+activeMemberCount+'명'):(todayLeaveCount+'명'),
        tone:todayLeaveCount&&leaveRatio>=0.3?'warning':'neutral',
        quiet:!(todayLeaveCount&&leaveRatio>=0.3),
        meta:todayLeaveCount
          ?('팀 일정에서 가용 '+availableMemberCount+'명 확인'+(todayFieldworkCount?' · 필드 '+todayFieldworkCount+'명':''))
          :('팀 일정에서 전체 '+activeMemberCount+'명 확인'+(todayFieldworkCount?' · 필드 '+todayFieldworkCount+'명':'')),
        action:"openDashDrilldown('leave')"
      }
    ];
    el.innerHTML='<div class="home-risk-grid">'+cards.map(renderHomeRiskSummaryCard).join('')+'</div>';
  }catch(e){
    const fallbackLabels=['오늘 마감','지연 중','내 이슈','미청구','오늘 휴가'];
    el.innerHTML='<div class="home-risk-grid">'
...
const HOME_WEEKDAY_LABELS=['일','월','화','수','목','금','토'];

function formatHomeMonthDayWithWeekday(dateLike,separator='.'){
  const d=typeof dateLike==='string'?toDate(dateLike):new Date(dateLike);
  if(!d||Number.isNaN(d.getTime()))return '';
  return (d.getMonth()+1)+separator+d.getDate()+'('+HOME_WEEKDAY_LABELS[d.getDay()]+')';
}

function formatHomeDateRangeWithWeekday(start,end,separator='.'){
  const startText=formatHomeMonthDayWithWeekday(start,separator);
  const endText=formatHomeMonthDayWithWeekday(end||start,separator);
  if(!startText)return '';
...
function buildHomeDailyProjectItem(project,kind,today){
  ...
  const projectName=project.name||'프로젝트 없음';
  ...
  return {
    ...
    contextLead:clientName,
    contextTitle:projectName,
    kind,
    badgeLabel:'프로젝트',
    badgeClass:'project',
...
function buildHomeDailyIssueItem(issue,today){
  ...
  return {
    ...
    contextTitle:project?.name||'프로젝트 없음',
    kind:'issue',
    badgeLabel:'이슈',
    badgeClass:'issue',
    context:(client?.name||'거래처 없음')+' — '+(project?.name||'프로젝트 없음'),
...
function buildHomeDailyScheduleItem(schedule,today){
  ...
  return {
    ...
    contextLead:memberLabel||'담당자 미지정',
    contextTitle:typeLabel||'일정',
    kind:'schedule',
...
function formatHomeWeekRangeLabel(start,end){
  return formatHomeDateRangeWithWeekday(start,end,'/');
}
...
function renderHomeMemberScheduleTagList(items){
  return items.length
    ?'<div class="home-team-member-tags">'+items.map(schedule=>'<button type="button" class="home-team-member-tag '+getHomeScheduleTone(schedule.schedule_type)+'" onclick="openScheduleModal(\''+schedule.id+'\')">'+esc(scheduleLabel(schedule.schedule_type))+' · '+esc(formatHomeDateRangeWithWeekday(schedule.start,schedule.end,'.'))+'</button>').join('')+'</div>'
...
async function renderTeamWorkload(){
  ...
  el.innerHTML='<div class="card home-card">'
    ...
    +'<div class="home-team-workload-head"><div class="home-subsection-title">팀 워크로드</div><div class="home-meta-text">업무가 많은 멤버 순으로 정렬</div></div>'
    +workloadHtml
  +'</div>';
}
...
async function renderHomeRiskSummaryLegacy(){
  const el=document.getElementById('homeRiskWrap');
  if(!el)return;
  el.innerHTML='<div class="home-risk-grid">'
  ...
  try{
    ...
    const normalizedCards=cards.map((card,index)=>{
      ...
      if(index===2){
        return {
          ...card,
          label:'확인 필요',
          meta:pendingDocRows.length?('프로젝트 문서 탭에서 '+pendingDocRows.length+'건 확인'):'기한 자료 요청 없음'
        };
      }
      if(index===3){
        return {
    ...
    el.innerHTML='<div class="home-risk-grid">'+normalizedCards.map(renderHomeRiskSummaryCard).join('')+'</div>';
  }catch(e){
    el.innerHTML='<div class="home-risk-grid">'
      +['오늘 마감','지연중','내 이슈','미청구','자료 대기','팀 가용성'].map(label=>
        '<div class="home-risk-card"><div class="home-risk-label">'+label+'</div><div class="home-risk-value">—</div><div class="home-risk-meta">상단 요약을 불러오지 못했습니다.</div></div>'
...
async function renderHomeRiskSummary(){
  const el=document.getElementById('homeRiskWrap');
  if(!el)return;
  const loadingLabels=['오늘 마감','지연중','자료 대기','내 이슈','미청구','오늘 휴가'];
  el.innerHTML='<div class="home-risk-grid">'
    +loadingLabels.map(label=>
      '<div class="home-risk-card"><div class="home-risk-label">'+label+'</div><div class="home-risk-value">-</div><div class="home-risk-meta">불러오는 중...</div></div>'
    ).join('')
  ...
  try{
    ...
    const todayLeaveCount=todayLeaveNames.length;
    const todayFieldworkCount=todayFieldworkNames.length;
    const activeMemberCount=activeMembers.length;
    const availableMemberCount=Math.max(activeMemberCount-todayLeaveCount,0);
    const leaveRatio=activeMemberCount?todayLeaveCount/activeMemberCount:0;
    const oldestOverdue=overdueProjects[0]||null;
    ...
    const cards=[
      ...
      },
      {
        ...
        quiet:!myOpenIssues.length,
        emphasis:'action',
        meta:myOpenIssues.length?(myHighPriorityIssues.length?('긴급 '+myHighPriorityIssues.length+'건 포함'):'담당 이슈 진행중'):(currentMember?.name?'내 담당 열린 이슈 없음':'담당자 정보 없음'),
        action:"setPage('issues')"
      },
      {
        label:'미청구',
      ...
      },
      {
        ...
        quiet:!(todayLeaveCount&&leaveRatio>=0.3),
        meta:todayLeaveCount
          ?('가용 '+availableMemberCount+'명 · '+todayLeaveNames[0]+(todayLeaveNames.length>1?' 외 '+(todayLeaveNames.length-1)+'명':'')+' 휴가'+(todayFieldworkCount?' · 필드웍 '+todayFieldworkCount+'명':''))
          :('전체 '+activeMemberCount+'명 · 오늘 휴가 없음'+(todayFieldworkCount?' · 필드웍 '+todayFieldworkCount+'명':'')),
        action:"openDashDrilldown('leave')"
      }
    ];
    ...
    el.innerHTML='<div class="home-risk-grid">'
      +['오늘 마감','지연중','자료 대기','내 이슈','미청구','오늘 휴가'].map(label=>
        '<div class="home-risk-card"><div class="home-risk-label">'+label+'</div><div class="home-risk-value">-</div><div class="home-risk-meta">상단 요약을 불러오지 못했습니다.</div></div>'
      ).join('')
      +'</div>';
  }
