...
function getProjectReferenceRows(source){
  return Array.isArray(source)?source:[];
}

function getActiveProjectTypeRows(){
  return getProjectReferenceRows(typeof projectTypes==='undefined'?[]:projectTypes)
...
function getProjectTypeRowById(typeId){
  const key=String(typeId||'').trim();
  if(!key)return null;
  return getProjectReferenceRows(typeof projectTypes==='undefined'?[]:projectTypes)
    .find(type=>String(type?.id||'')===key)||null;
}

function getProjectTeamRowById(teamId){
  const key=String(teamId||'').trim();
  if(!key)return null;
  return getProjectReferenceRows(typeof referenceTeams==='undefined'?[]:referenceTeams)
    .find(team=>String(team?.id||'')===key)||null;
}
...
function getCurrentMemberProjectTeamId(){
  const member=typeof currentMember==='undefined'?null:currentMember;
  const direct=String(member?.team_id||'').trim();
  if(direct)return direct;
  const teamName=String(member?.team||'').trim();
  if(!teamName)return '';
  return String(getActiveProjectTeamRows().find(team=>String(team?.name||'').trim()===teamName)?.id||'');
}
...
function getVisibleProjectTeamRows(){
  const rows=getActiveProjectTeamRows();
  if(canViewAllProjectTeams())return rows;
  const ownTeamId=getCurrentMemberProjectTeamId();
  return ownTeamId?rows.filter(team=>String(team?.id||'')===ownTeamId):[];
}

function ensureGanttTeamFilterDefault(){
  if(canViewAllProjectTeams())return;
  if(!ganttTeamFilterId){
    ganttTeamFilterId=getCurrentMemberProjectTeamId();
  }
}

function getGanttTeamFilterValue(){
  ensureGanttTeamFilterDefault();
  return String(ganttTeamFilterId||'all').trim()||'all';
}

function setGanttTeamFilter(value){
  const normalized=String(value||'all').trim()||'all';
  ganttTeamFilterId=normalized==='all'?'':normalized;
  renderGantt();
}
...
function getProjectTeamSelectOptions(selectedId=''){
  const rows=getActiveProjectTeamRows();
  const selected=String(selectedId||'').trim();
  const currentRow=selected?getProjectTeamRowById(selected):null;
  const options=[...rows];
  if(currentRow&&!options.some(row=>String(row?.id||'')===selected))options.unshift(currentRow);
  return '<option value="">팀 선택</option>'
...
function getGanttTypeFilterLabel(value){
  const raw=String(value||'').trim();
  if(!raw||raw==='all')return '전체';
  return getProjectTypeRowById(raw)?.name||raw;
}
...
function projectMatchesTopFilters(project){
  ...
  if(ganttTypeFilters.length){
    ...
    if(!ganttTypeFilters.includes(typeId)&&!ganttTypeFilters.includes(typeLabel))return false;
  }
...
function refreshGanttActiveViewOnly(){
  ...
  if(curGanttLayout==='timeline'&&(curGanttTimelineScale==='annual'||curGanttTimelineScale==='week')&&typeof renderGanttAnnualTimeline==='function'){
    renderGanttAnnualTimeline(currentData.projs);
    return;
  }
  ...
  bar.innerHTML=''
    ...
    +'<div class="gantt-top-filter-group gantt-type-group">'
      ...
      +'<div class="gantt-type-chip-list">'
        +(getActiveProjectTypeRows().length
          ?getActiveProjectTypeRows().map(type=>'<button type="button" class="gantt-type-chip'+(ganttTypeFilters.includes(String(type.id))?' active':'')+'" onclick="toggleGanttTypeFilter(\''+esc(type.id)+'\')">'+esc(type.name||type.code||'유형')+'</button>').join('')
...
function getGanttOverviewTaskKpiStats(projs){
  const ids=(Array.isArray(projs)?projs:[]).map(project=>String(project?.id||'')).filter(Boolean);
  return ids.reduce((acc,id)=>{
    const summary=ganttListTaskSummaryByProjectId[id];
    if(!summary)return acc;
    acc.overdue+=Number(summary.overdueCount||0);
    acc.dueThisWeek+=Number(summary.dueThisWeekCount||0);
    return acc;
...
function setGanttCalendarDisplayMode(mode){
  ganttCalendarDisplayMode=mode==='compact'?'compact':'full';
  if(typeof renderGantt==='function')renderGantt();
}

function scheduleGanttCalendarTaskRefresh(){
  if(ganttCalendarTaskRefreshTimer)return;
  ganttCalendarTaskRefreshTimer=setTimeout(()=>{
    ganttCalendarTaskRefreshTimer=0;
    if(curGanttLayout==='calendar'&&typeof renderGantt==='function')renderGantt();
  },0);
}

function ensureGanttCalendarProjectTasksLoaded(projs){
  if(curGanttLayout!=='calendar')return;
  (projs||[]).forEach(project=>{
    const key=String(project?.id||'').trim();
    if(!key)return;
    const loadMeta=getGanttProjectTaskLoadMeta(key);
    if(Array.isArray(ganttProjectTasksByProjectId[key])||loadMeta.loading)return;
    loadGanttProjectTasks(key,false).then(scheduleGanttCalendarTaskRefresh).catch(scheduleGanttCalendarTaskRefresh);
  });
}

function buildGanttCalendarTaskItem(project,task,isDueDay,dateValue){
  ...
  const title=String(task?.title||'제목 없는 업무');
  return {
    kind:'task',
    id:task?.id,
    projectId,
    label:(isDueDay?'업무 · ':'시작 · ')+title,
    title:[
      project?.name||'프로젝트',
      title,
      assignee,
      task?.status||'예정',
      (isDueDay?'기한 ':'시작 ')+formatGanttTaskShortDate(dateValue),
...
function getGanttCalendarTaskItemsForProjectAndDate(project,cellDate){
  const projectId=String(project?.id||'').trim();
  if(!projectId)return [];
  const dayValue=getGanttCalendarDateValue(cellDate);
  if(!dayValue)return [];
  const tasks=Array.isArray(ganttProjectTasksByProjectId[projectId])?ganttProjectTasksByProjectId[projectId]:[];
  return tasks.reduce((rows,task)=>{
    if(String(task?.status||'').trim()==='완료')return rows;
    ...
    rows.push(buildGanttCalendarTaskItem(project,task,isDueDay,isDueDay?dueValue:startValue));
    return rows;
  },[]);
}
...
function buildGanttCalendarItemMap(projs,schs){
  ...
  (projs||[]).forEach(project=>{
    ...
    const tasks=Array.isArray(ganttProjectTasksByProjectId[String(project.id||'')])
      ...
      :[];
    tasks.forEach(task=>{
      if(String(task?.status||'').trim()==='완료')return;
      const dueValue=getGanttTaskDateValue(task?.due_date);
      const startValue=getGanttTaskDateValue(task?.start_date);
      const dateValue=dueValue||startValue;
      if(!dateValue)return;
      const taskDate=toDate(dateValue);
      if(Number.isNaN(taskDate.getTime())||taskDate<monthFirst||taskDate>monthLast)return;
      pushItem(dateValue,buildGanttCalendarTaskItem(project,task,Boolean(dueValue),dateValue));
    });
  });
...
function renderGanttCalendarGrid(projs,schs){
  ...
  for(let i=0;i<totalCells;i++){
    ...
    const isCompact=ganttCalendarDisplayMode==='compact';
    const visibleItems=isCompact?items.slice(0,4):items;
    const preview=visibleItems.map(buildGanttCalendarItemHtml).join('');
    const overflow=isCompact&&items.length>4?'<div class="gantt-calendar-item more">+'+(items.length-4)+'건</div>':'';
  ...
  wrap.innerHTML=''
    ...
    +'<div class="gantt-calendar-viewbar">'
      +'<div class="gantt-calendar-viewbar-copy">'
        ...
        +'<div class="gantt-calendar-viewbar-sub">'+(ganttCalendarDisplayMode==='compact'?'하루 4개까지 표시하고 나머지는 접습니다.':'각 날짜의 프로젝트와 업무를 모두 표시합니다.')+'</div>'
      +'</div>'
      +'<div class="toggle-wrap gantt-calendar-view-toggle">'
        +'<button type="button" class="toggle-btn'+(ganttCalendarDisplayMode==='compact'?' active':'')+'" onclick="setGanttCalendarDisplayMode(\'compact\')">간략 보기</button>'
        +'<button type="button" class="toggle-btn'+(ganttCalendarDisplayMode!=='compact'?' active':'')+'" onclick="setGanttCalendarDisplayMode(\'full\')">전체 보기</button>'
      +'</div>'
    +'</div>'
  ...
  wrap.innerHTML='<div class="gantt-list-view">'
    +sorted.map(project=>{
      ...
      return '<button type="button" class="gantt-list-card" onclick="openProjModal(\''+project.id+'\')">'
        +'<div class="gantt-list-card-head"><div><div class="gantt-list-card-title">'+esc(project.name)+'</div><div class="gantt-list-card-sub">'+esc(client?.name||'고객사 미지정')+' · '+esc(getProjectTypeLabel(project))+' · '+esc(getProjectTeamLabel(project))+'</div></div><span class="badge '+(project.status==='진행중'?'badge-blue':project.status==='완료'?'badge-green':'badge-orange')+'">'+esc(project.status||'상태 없음')+'</span></div>'
...
function getGanttListTaskSummaryText(summary){
  ...
  if(summary.overdueCount>0)parts.push('지연 업무 '+summary.overdueCount+'건');
...
async function loadGanttListTaskSummaries(projectIds){
  ...
  try{
    const rows=await api(
      'GET',
      GANTT_PROJECT_TASK_TABLE+'?project_id=in.('+missingIds.join(',')+')&select=project_id,title,status,due_date,assignee_member_id,actual_done_at'
    );
    const grouped={};
  ...
  const currentData=getGanttFilteredData();
  renderGanttOverviewCards(currentData.projs,currentData.schs);
...
function getGanttListRiskMeta(project,issueCount,taskSummary,taskIssueSummary,pendingDocSummary){
  ...
  if(Number(taskSummary?.overdueCount||0)>0)return {label:'지연 업무',tone:'warn',rank:3.8,detail:'지연 업무 '+taskSummary.overdueCount+'건'};
...
function getGanttListProjectRows(projs){
  return (projs||[]).map(project=>{
    ...
    const typeText=getProjectTypeLabel(project);
    const teamText=getProjectTeamLabel(project);
    const searchText=[clientName,project?.name||'',typeText,teamText,project?.type||'',memberNames.join(' ')].join(' ').toLowerCase();
...
function getGanttListExecutionSignalItems(row){
  ...
  if(Number(row?.taskSummary?.overdueCount||0)>0&&row?.riskMeta?.label!=='지연 업무')items.push({label:'지연 업무 '+row.taskSummary.overdueCount+'건',tone:'warn'});
...
function getGanttListSignalBarMarkup(overdueRows,dueTodayRows,issueAttentionRows,overdueTaskCount){
  const chips=[];
  if(overdueRows.length)chips.push('<div class="gantt-list-signal-chip is-danger">지연 프로젝트 '+overdueRows.length+'건</div>');
  if(dueTodayRows.length)chips.push('<div class="gantt-list-signal-chip is-warn">오늘 마감 '+dueTodayRows.length+'건</div>');
  if(Number(overdueTaskCount||0)>0)chips.push('<div class="gantt-list-signal-chip is-warn">지연 업무 '+overdueTaskCount+'건</div>');
...
function getGanttListExecutionRiskFilterOptions(){
  return [
    {value:'overdue_task',label:'지연 업무'},
  ...
  return ''
    +'<div class="gantt-detail-pane">'
      ...
      +'<div class="gantt-detail-section">'
        +'<div class="gantt-inline-note-top-actions"><button type="button" class="gantt-detail-link" onclick="startGanttDetailInlineMemoEdit(\''+project.id+'\',\'memo\')">수정</button></div>'
...
async function completeProjectTask(projectId,taskId){
  ...
  try{
    ...
    console.error('completeProjectTask failed',error);
    alert(isMissingGanttProjectTaskTableError(error)
      ?getMissingGanttProjectTaskTableMessage()
...
async function loadGanttDetailAsync(project){
  ...
  if(!projectId)return;
  try{
    const issueStatusFilter=ganttDetailTab==='issues'
      ?''
      :'&'+(typeof getIssueActiveStatusFilter==='function'?getIssueActiveStatusFilter():'status=neq.resolved');
    const [issueRows,documentRows,taskRows]=await Promise.all([
      api('GET','project_issues?project_id=eq.'+projectId+issueStatusFilter+'&select=id,project_id,task_id,title,status,priority,is_pinned,assignee_name,assignee_member_id,owner_name,created_at').catch(()=>[]),
      api('GET','document_requests?project_id=eq.'+projectId+'&status=eq.pending&select=id,project_id,title,due_date,sort_order&order=sort_order.asc').catch(()=>[]),
  ...
  el.innerHTML=''
    +'<div class="gantt-detail-header">'
      +'<div class="gantt-detail-head-copy">'
        ...
        +'</div>'
        +'<div class="gantt-detail-badges"><span class="badge '+getGanttDetailTypeBadgeClass(getProjectTypeLabel(project))+'">'+esc(getProjectTypeLabel(project))+'</span><span class="badge badge-gray">'+esc(getProjectTeamLabel(project))+'</span><span class="badge '+getGanttDetailStatusBadgeClass(project.status)+'">'+esc(project.status||'예정')+'</span>'+priorityBadge+'</div>'
      +'</div>'
...
function getGanttDetailRenderSignature(projs){
  ...
  const baseParts=[
    ...
    String(project.project_type_id||''),
    ...
    String(project.memo||''),
    String(project.work_summary||''),
    String(project.result_summary||''),
    String(project.issue_note||''),
    String(project.follow_up_needed||''),
    String(project.follow_up_note||''),
    String(project.updated_at||''),
    String(ganttDetailInlineEditKey||''),
    String(ganttDetailInlineSaveMessage||'')
  ];
  if(ganttDetailTab==='overview'){
  ...
  }else if(ganttDetailTab==='checklist'){
    baseParts.push(JSON.stringify(getGanttProjectChecklistState(project.id)||{}));
  }
...
function getGanttListAttentionSubtext(row){
  if(Number(row?.taskSummary?.overdueCount||0)>0)return '지연 업무 '+row.taskSummary.overdueCount+'건';
...
function setGanttDetailIssueSaveMessage(projectId,message){
  if(String(projectId||'')!==String(ganttFocusProjectId||''))return;
  ganttDetailIssueSaveMessage=String(message||'');
  const detail=document.getElementById('ganttDetail');
  if(detail)detail.dataset.renderSignature='';
}

function clearGanttDetailIssueSaveMessage(message){
  if(message&&ganttDetailIssueSaveMessage!==message)return;
  ganttDetailIssueSaveMessage='';
  const detail=document.getElementById('ganttDetail');
  if(detail&&String(detail.dataset.activeTab||'')==='issues'){
    detail.dataset.renderSignature='';
    if(typeof renderGantt==='function')renderGantt();
  }
}

function renderGanttIssueAddButton(projectId,classes='btn primary sm'){
  return '<button type="button" class="'+classes+'" onclick="event.stopPropagation();openGanttProjectIssueModal(\''+projectId+'\')">+ 이슈 추가</button>';
}

function openGanttProjectIssueModal(projectId,issueId=''){
  ganttIssueModalProjectId=issueId?'':String(projectId||'');
  if(typeof openIssueModal==='function')openIssueModal(projectId,issueId);
}

function renderGanttIssueEmptyState(projectId){
  return ''
    +'<div class="gantt-detail-empty-state">'
      +'<div class="gantt-detail-value">등록된 프로젝트 이슈가 없습니다.</div>'
      +'<div class="gantt-detail-meta">프로젝트 단위 이슈나 특정 업무에 연결된 이슈를 추가해 진행 리스크를 관리할 수 있습니다.</div>'
      +'<div>'+renderGanttIssueAddButton(projectId)+'</div>'
    +'</div>';
}

function renderGanttDetailIssueItem(projectId,issue){
  const statusMeta=typeof getIssueStatusMeta==='function'?getIssueStatusMeta(issue.status):{label:'상태',badgeCls:'badge-blue'};
  const editable=typeof canEditIssue==='function'?canEditIssue(issue):false;
  const isResolved=typeof isIssueResolvedStatus==='function'?isIssueResolvedStatus(issue.status):String(issue?.status||'')==='resolved';
  const canResolve=editable&&!isResolved;
...
function renderGanttDetailIssuePreview(projectId,issues){
  ...
  if(ganttDetailTab!=='issues'){
    const activeRows=rows.filter(issue=>!(typeof isIssueResolvedStatus==='function'?isIssueResolvedStatus(issue.status):String(issue?.status||'')==='resolved'));
    container.innerHTML=(activeRows.length?activeRows:rows).map(issue=>renderGanttDetailIssueItem(projectId,issue)).join('');
    return;
  }
  const activeIssues=rows.filter(issue=>!(typeof isIssueResolvedStatus==='function'?isIssueResolvedStatus(issue.status):String(issue?.status||'')==='resolved'));
...
function renderGanttProjectIssuesSection(project){
  ...
  return ''
    +'<div class="gantt-detail-pane">'
      +'<div class="gantt-detail-section gantt-detail-section--flush">'
        +'<div class="gantt-detail-section-head"><div><div class="gantt-panel-title">프로젝트 이슈</div><div class="gantt-detail-meta">프로젝트 전체 리스크와 업무 연결 이슈를 함께 확인합니다. 행이나 제목을 클릭하면 수정할 수 있습니다.</div></div><div class="gantt-detail-head-actions">'+renderGanttIssueAddButton(projectId)+'<button type="button" class="gantt-detail-link" onclick="openProjModal(\''+projectId+'\',null,null,\'issue\')">전체 이슈 보기</button></div></div>'
        +'<div class="gantt-detail-list" id="ganttDetailIssueList">'+(ganttDetailIssueSaveMessage?'<div class="gantt-detail-save-message">'+esc(ganttDetailIssueSaveMessage)+'</div>':'')+'<div class="gantt-detail-empty">불러오는 중...</div></div>'
      +'</div>'
    +'</div>';
}
...
function getGanttDetailInlineEditKey(projectId,field){
  return String(projectId||'')+':'+String(field||'');
}

function getGanttDetailInlineEditMeta(project){
  const updatedValue=project?.updated_at||project?.modified_at||project?.created_at||'';
  const updatedText=updatedValue&&typeof formatCommentDate==='function'
    ?formatCommentDate(updatedValue)
    :String(updatedValue||'');
  const actor=project?.updated_by_name||project?.modified_by_name||project?.author_name||currentMember?.name||currentUser?.email||'기록 없음';
  return '최종 수정: '+actor+(updatedText?' / '+updatedText:'');
}

function renderGanttDetailInlineMemoEditor(project,config){
  const projectId=String(project?.id||'');
  const field=String(config?.field||'memo');
  const key=getGanttDetailInlineEditKey(projectId,field);
  const value=String(project?.[field]||'');
  const hasValue=!!value.trim();
  const isEditing=ganttDetailInlineEditKey===key;
  const title=String(config?.title||'메모');
  const emptyText=String(config?.emptyText||'등록된 메모가 없습니다.');
  const placeholder=String(config?.placeholder||'메모를 입력하세요.');
  const canEdit=typeof canManageGanttListProject==='function'?canManageGanttListProject(project):true;
  const success=ganttDetailInlineSaveMessage===key;
  if(isEditing){
    return ''
      +'<div class="gantt-inline-note is-editing" data-inline-note="'+esc(key)+'">'
        +'<label class="gantt-inline-note-label" for="ganttInlineNoteInput">'+esc(title)+'</label>'
        +'<textarea id="ganttInlineNoteInput" class="gantt-inline-note-input" placeholder="'+esc(placeholder)+'">'+esc(value)+'</textarea>'
        +'<div class="gantt-inline-note-footer">'
          +'<div class="gantt-inline-note-meta">'+esc(getGanttDetailInlineEditMeta(project))+'</div>'
          +'<div class="gantt-inline-note-actions">'
            +'<button type="button" class="btn ghost sm" onclick="cancelGanttDetailInlineMemoEdit()">취소</button>'
            +'<button type="button" class="btn primary sm" onclick="saveGanttDetailInlineMemo(\''+projectId+'\',\''+field+'\')">저장</button>'
          +'</div>'
        +'</div>'
      +'</div>';
  }
  return ''
    +'<div class="gantt-inline-note'+(hasValue?'':' is-empty')+'" '+(canEdit?'onclick="startGanttDetailInlineMemoEdit(\''+projectId+'\',\''+field+'\')" tabindex="0" role="button" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();startGanttDetailInlineMemoEdit(\''+projectId+'\',\''+field+'\')}"':'')+'>'
      +'<div class="gantt-inline-note-body">'
        +(hasValue
          ?'<div class="gantt-inline-note-text">'+esc(value)+'</div>'
          :'<div class="gantt-inline-note-empty">'+esc(emptyText)+'</div>')
        +(canEdit?'<div class="gantt-inline-note-hint">클릭하여 수정</div>':'')
      +'</div>'
      +'<div class="gantt-inline-note-footer">'
        +'<div class="gantt-inline-note-meta">'+esc(getGanttDetailInlineEditMeta(project))+'</div>'
        +'<div class="gantt-inline-note-actions">'
          +(canEdit?'<button type="button" class="btn ghost sm" onclick="event.stopPropagation();startGanttDetailInlineMemoEdit(\''+projectId+'\',\''+field+'\')">'+(hasValue?'수정':'+ 메모 작성')+'</button>':'')
        +'</div>'
      +'</div>'
      +(success?'<div class="gantt-inline-note-success">저장되었습니다.</div>':'')
    +'</div>';
}

function refreshGanttDetailInlineMemoPanel(projectId){
  const el=document.getElementById('ganttDetail');
  if(el)el.dataset.renderSignature='';
  const currentData=typeof getGanttFilteredData==='function'?getGanttFilteredData():{projs:projects||[],schs:schedules||[]};
  renderGanttDetailPanel(currentData.projs||[],currentData.schs||[]);
}

function startGanttDetailInlineMemoEdit(projectId,field){
  ganttDetailInlineEditKey=getGanttDetailInlineEditKey(projectId,field);
  ganttDetailInlineSaveMessage='';
  refreshGanttDetailInlineMemoPanel(projectId);
  requestAnimationFrame(()=>{
    const input=document.getElementById('ganttInlineNoteInput');
    if(input){
      input.focus();
...
function cancelGanttDetailInlineMemoEdit(){
  const key=ganttDetailInlineEditKey;
  ganttDetailInlineEditKey='';
  refreshGanttDetailInlineMemoPanel(key.split(':')[0]||ganttFocusProjectId);
}

async function saveGanttDetailInlineMemo(projectId,field){
  const key=getGanttDetailInlineEditKey(projectId,field);
  const input=document.getElementById('ganttInlineNoteInput');
  const nextValue=input?input.value.trim():'';
  const patchBody={[field]:nextValue||null,updated_at:new Date().toISOString()};
  try{
    ...
    if(idx!==-1)projects[idx]={...projects[idx],...patchBody};
    ganttDetailInlineEditKey='';
    ganttDetailInlineSaveMessage=key;
    refreshGanttDetailInlineMemoPanel(projectId);
    setTimeout(()=>{
      if(ganttDetailInlineSaveMessage===key){
        ganttDetailInlineSaveMessage='';
        refreshGanttDetailInlineMemoPanel(projectId);
      }
    },2200);
    if(typeof logActivity==='function'){
      const project=(projects||[]).find(project=>String(project?.id||'')===String(projectId||''));
      logActivity('프로젝트 메모 수정','project',projectId,project?.name||'').catch(()=>{});
    }
  }catch(error){
  ...
  bar.innerHTML=''
    ...
    +'<div class="gantt-top-filter-group gantt-type-group">'
      ...
      +'<select id="ganttTypeFilterSelect">'
        ...
        +(getActiveProjectTypeRows().length
          ?getActiveProjectTypeRows().map(type=>'<option value="'+esc(type.id)+'"'+(typeFilterValue===String(type.id)?' selected':'')+'>'+esc(type.name||type.code||'유형')+'</option>').join('')
...
function projectMatchesTopFilters(project){
  ...
  if(typeFilterValue!=='all'){
    const typeId=String(project?.project_type_id||'').trim();
    const typeLabel=getProjectTypeLabel(project);
    if(typeFilterValue!==typeId&&typeFilterValue!==typeLabel)return false;
  }
  const teamFilterValue=getGanttTeamFilterValue();
  if(teamFilterValue!=='all'&&teamFilterValue!==String(project?.team_id||''))return false;
  if(ganttClientFilterQuery){
    const clientName=getGanttProjectClientName(project).toLowerCase();
    if(!clientName.includes(String(ganttClientFilterQuery).trim().toLowerCase()))return false;
  }
...
function buildGanttListTaskSummary(tasks){
  ...
  rows.forEach(task=>{
    ...
    const isDone=status==='완료'||!!String(task?.actual_done_at||'').trim();
    const dueMeta=getGanttTaskDueMeta(task);
    if(!isDone&&dueMeta.tone==='danger')overdueCount+=1;
    if(!isDone&&dueMeta.label==='오늘 마감')dueTodayCount+=1;
    else if(!isDone&&dueMeta.tone==='warn')dueSoonCount+=1;
    const dueDiff=typeof getGanttTaskDueDiff==='function'?getGanttTaskDueDiff(task):null;
    if(!isDone&&dueDiff!=null&&dueDiff>=0&&dueDiff<=6)dueThisWeekCount+=1;
    if(!isDone){
      openCount+=1;
      if(status==='대기'||status==='보류')waitingCount+=1;
      if(status==='진행중')inProgressCount+=1;
...
function getGanttListRiskMeta(project,issueCount,taskSummary,taskIssueSummary,pendingDocSummary){
  ...
  if(Number(taskSummary?.overdueCount||0)>0)return {label:'지연 업무',tone:'warn',rank:4.3,detail:'열린 업무 중 기한이 지난 항목이 있습니다.'};
...
function getGanttListExecutionSignalItems(row){
  ...
  if(overdueTaskCount>0)items.push({label:'지연 업무 '+overdueTaskCount+'건',tone:'warn'});
...
function getGanttListPriorityState(row){
  ...
  if(overdueTaskCount>0){
    return {label:'지연 업무',tone:'warn',order:100};
  }
  if(lifecycleKey==='overdue'||isGanttProjectOverdue(row?.project)){
    return {label:'일정 점검',tone:'danger',order:96};
  }
...
function getGanttListActionCue(row){
  ...
  const nextDueMeta=getGanttListNextDueMeta(row);
  if(overdueTaskCount>0){
    return {label:'지연 업무 확인',tone:'warn'};
  }
  if(lifecycleKey==='overdue'||isGanttProjectOverdue(row?.project)){
...
function getGanttListSecondarySignals(row){
  ...
  const nextDueMeta=getGanttListNextDueMeta(row);
  if(Number(row?.taskSummary?.overdueCount||0)>0&&priorityLabel!=='지연 업무')items.push({label:'지연 업무 '+row.taskSummary.overdueCount+'건',tone:'warn'});
...
function renderGanttListPriorityQueueBar(rows){
  const priorityCounts={
    overdueProjects:rows.filter(row=>{
      const lifecycleKey=String(row?.lifecycleMeta?.key||'').trim();
      return lifecycleKey==='overdue'||isGanttProjectOverdue(row?.project);
    }).length,
    overdueTasks:rows.filter(row=>Number(row?.taskSummary?.overdueCount||0)>0).length,
  ...
  const chips=[];
  if(priorityCounts.overdueProjects)chips.push('<div class="gantt-list-signal-chip is-danger">지연 프로젝트 '+priorityCounts.overdueProjects+'건</div>');
  if(priorityCounts.overdueTasks)chips.push('<div class="gantt-list-signal-chip is-warn">지연 업무 '+priorityCounts.overdueTasks+'건</div>');
...
function getGanttListClientGroupKey(row){
  const clientId=String(row?.project?.client_id||'').trim();
  if(clientId)return 'client:'+clientId;
  return 'name:'+(String(row?.clientName||'거래처 미지정').trim()||'거래처 미지정');
}

function getGanttListClientGroups(rows){
  const groupMap=new Map();
  (Array.isArray(rows)?rows:[]).forEach(row=>{
    const key=getGanttListClientGroupKey(row);
    const clientName=String(row?.clientName||'거래처 미지정').trim()||'거래처 미지정';
    if(!groupMap.has(key)){
      groupMap.set(key,{
        key,
        clientName,
    ...
    group.rows.push(row);
    if(isGanttProjectOverdue(row?.project)||String(row?.lifecycleMeta?.key||'')==='overdue')group.overdueProjects+=1;
    group.overdueTasks+=Number(row?.taskSummary?.overdueCount||0);
    const nextDue=getGanttListNextDueValue(row)?.value||'';
    if(nextDue&&(!group.nextDueValue||String(nextDue).localeCompare(group.nextDueValue)<0)){
      group.nextDueValue=String(nextDue);
...
function hasActiveGanttListFilters(){
  ...
  return !!String(ganttListSearchQuery||'').trim()
    ...
    ||String(getGanttTeamFilterValue()||'all')!=='all'
...
renderGanttTopFilterBar=function(){
  ...
  if(teamSelect)teamSelect.onchange=e=>setGanttTeamFilter(e.target.value);
  const clientInput=document.getElementById('ganttClientFilterInput');
  if(clientInput){
    clientInput.onchange=e=>setGanttClientFilter(e.target.value);
    clientInput.oninput=e=>{
  ...
  if(typeof updateGanttLayoutButtons==='function')updateGanttLayoutButtons();
};
...
renderGanttActiveFilterTags=function(){
  ...
  if(teamFilterValue!=='all'){
    ...
    tags.push({kind:'team',value:teamFilterValue,label:'팀 '+teamLabel});
  }
  if(ganttClientFilterQuery)tags.push({kind:'client',value:ganttClientFilterQuery,label:'고객사 '+ganttClientFilterQuery});
  if(!tags.length){
    row.innerHTML='';
...
function getGanttTaskDrilldownJsString(value){
  return String(value??'').replace(/\\/g,'\\\\').replace(/'/g,"\\'").replace(/\r/g,'\\r').replace(/\n/g,'\\n');
}

function getGanttTaskDoneText(task){
  const raw=String(task?.actual_done_at||'').trim();
  if(!raw)return '';
...
function getGanttTaskDrilldownDueText(task){
  if(!getGanttTaskDateValue(task?.due_date))return '마감일 없음';
  return getGanttTaskDueMeta(task)?.label||getGanttTaskDateDisplayMeta(task).dueText||task.due_date;
}

function getGanttTaskDrilldownStatusMeta(task,dueMeta){
  const status=String(task?.status||'예정').trim()||'예정';
  if(status==='완료')return {label:'완료',className:'badge-green'};
  if(dueMeta?.tone==='danger')return {label:'지연',className:'badge-red'};
  return {label:status,className:getGanttTaskStatusBadgeClass(status)};
}

function getGanttTaskDrilldownPriorityText(task){
  const priority=String(task?.priority||'').trim();
  if(!priority)return '';
  if(typeof getGanttTaskPriorityLabel==='function')return getGanttTaskPriorityLabel(priority);
  return priority;
}

function getGanttListProjectTaskButtonText(row,isExpanded){
  if(isExpanded)return '업무 접기';
  const projectId=String(row?.project?.id||'').trim();
  const loadedTasks=Array.isArray(ganttProjectTasksByProjectId[projectId])?ganttProjectTasksByProjectId[projectId]:null;
  const count=loadedTasks?loadedTasks.length:Number(row?.taskSummary?.total||0);
...
function getGanttListDrilldownTasks(projectId){
  ...
  return [...getGanttProjectTasks(projectId)].sort((a,b)=>{
    const aDone=String(a?.status||'')==='완료';
    const bDone=String(b?.status||'')==='완료';
    if(aDone!==bDone)return aDone?1:-1;
    const aDue=getGanttTaskDateValue(a?.due_date)||'9999-12-31';
    const bDue=getGanttTaskDateValue(b?.due_date)||'9999-12-31';
...
renderGanttListTaskDrilldownRow=function(row){
  ...
  const tasks=hasLoadedTasks?getGanttListDrilldownTasks(projectId):[];
  let bodyHtml='';
  if(loadMeta.loading&&!hasLoadedTasks){
    ...
    bodyHtml='<div class="gantt-list-task-drill-empty">등록된 업무가 없습니다.</div>';
  }else{
    bodyHtml='<div class="gantt-list-task-drill-items">'
      +tasks.map(task=>renderGanttListTaskDrilldownItem(projectId,task)).join('')
    +'</div>';
  }
  return '<tr class="gantt-list-child-row">'
...
function getGanttProjectChecklistState(projectId){
  const key=String(projectId||'');
  return {
    meta:ganttProjectChecklistLoadMetaByProjectId[key]||{},
    checklist:ganttProjectChecklistByProjectId[key]?.checklist||null,
    items:ganttProjectChecklistByProjectId[key]?.items||[],
    templateCount:ganttChecklistTemplates.length,
    templatesLoaded:ganttChecklistTemplatesLoaded
  };
}

function invalidateGanttProjectChecklistRender(projectId){
  const el=document.getElementById('ganttDetail');
  if(el&&String(el.dataset.projectId||'')===String(projectId||'')){
    el.dataset.renderSignature='';
  }
}
...
async function loadGanttChecklistTemplates(force=false){
  ...
  try{
    ...
    ganttChecklistTemplates=[];
    ganttChecklistTemplatesLoaded=true;
  }
  return ganttChecklistTemplates;
}

async function loadGanttProjectChecklist(projectId,force=false){
  const key=String(projectId||'');
  if(!key)return;
  const state=ganttProjectChecklistByProjectId[key];
  const meta=ganttProjectChecklistLoadMetaByProjectId[key]||{};
  if(state&&!force)return;
  if(meta.loading)return;
  ganttProjectChecklistLoadMetaByProjectId[key]={loading:true,error:''};
  invalidateGanttProjectChecklistRender(key);
  try{
    await loadGanttChecklistTemplates();
    ...
    ganttProjectChecklistByProjectId[key]={checklist:null,items:[]};
    ganttProjectChecklistLoadMetaByProjectId[key]={loading:false,error:'체크리스트를 불러오지 못했습니다. migration 적용 후 다시 시도해 주세요.'};
  }
  invalidateGanttProjectChecklistRender(key);
  if(ganttDetailTab==='checklist'&&String(ganttFocusProjectId||'')===key)renderGantt();
}
...
function renderGanttChecklistStatusOptions(value){
  const current=String(value||'not_started');
  return GANTT_CHECKLIST_STATUSES.map(status=>
    '<option value="'+status.value+'" '+(current===status.value?'selected':'')+'>'+esc(status.label)+'</option>'
  ).join('');
}

function renderGanttChecklistAssigneeOptions(value){
  ...
  return '<option value="">미지정</option>'+memberOptions;
}

function renderGanttChecklistTemplateOptions(project){
  const projectType=String(getProjectTypeLabel(project)||project?.service_type||'').trim();
  const rows=[...(ganttChecklistTemplates||[])].sort((a,b)=>{
    const aMatch=projectType&&String(a?.service_type||'')===projectType?0:1;
    const bMatch=projectType&&String(b?.service_type||'')===projectType?0:1;
...
function openGanttChecklistTemplateModal(projectId){
  ...
  loadGanttChecklistTemplates(true).then(()=>{
    ...
    document.getElementById('modalArea').innerHTML=''
      +'<div class="overlay" onclick="if(event.target===this)closeModal()">'
        +'<div class="modal" style="max-width:560px">'
          +'<div class="modal-head"><div><h2>체크리스트 적용</h2><p class="modal-sub">활성 템플릿을 선택하면 항목이 이 프로젝트로 복사됩니다.</p></div><button class="modal-x" onclick="closeModal()">×</button></div>'
          +(options
            ?'<div class="form-row"><label class="form-label">템플릿</label><select id="ganttChecklistTemplateSelect">'+options+'</select></div>'
            :'<div class="gantt-detail-empty">활성화된 체크리스트 템플릿이 없습니다. 먼저 템플릿을 등록해 주세요.</div>')
          +'<div class="modal-footer"><div></div><div class="modal-footer-right"><button class="btn ghost" onclick="closeModal()">취소</button>'+(options?'<button class="btn primary" onclick="applyGanttChecklistTemplate(\''+projectId+'\')">적용</button>':'')+'</div></div>'
        +'</div>'
      +'</div>';
  });
}
...
async function applyGanttChecklistTemplate(projectId){
  ...
  try{
    const existing=await api('GET','project_checklists?project_id=eq.'+projectId+'&status=neq.archived&select=id&limit=1')||[];
    if(existing.length&&!confirm('이미 적용된 체크리스트가 있습니다. 새 체크리스트를 추가로 적용할까요?'))return;
    const created=await api('POST','project_checklists',{
      project_id:projectId,
      template_id:templateId,
      status:'active',
    ...
    const checklist=created[0];
    if(!checklist?.id)throw new Error('project_checklists 생성 결과를 확인할 수 없습니다.');
    const templateItems=await api('GET','checklist_template_items?template_id=eq.'+templateId+'&select=*&order=section.asc,sort_order.asc,created_at.asc')||[];
    const rows=templateItems.map(item=>({
      project_checklist_id:checklist.id,
      project_id:projectId,
      template_item_id:item.id,
    ...
    delete ganttProjectChecklistByProjectId[String(projectId)];
    await loadGanttProjectChecklist(projectId,true);
    if(typeof showToast==='function')showToast('체크리스트가 적용되었습니다.','success');
  }catch(error){
    console.error('[projects] apply checklist template failed',error);
    alert('체크리스트 적용 실패: '+error.message);
  }
}

function renderGanttChecklistEmptyState(project){
  return '<div class="gantt-detail-empty-state">'
    +'<div class="gantt-detail-value">적용된 체크리스트가 없습니다.</div>'
    +'<div class="gantt-detail-meta">반복 업무 템플릿을 프로젝트에 적용하면 섹션별 확인 항목과 완료율을 관리할 수 있습니다.</div>'
    +'<div><button type="button" class="btn primary sm" onclick="openGanttChecklistTemplateModal(\''+project.id+'\')">체크리스트 적용</button></div>'
  +'</div>';
}
...
function renderGanttChecklistItems(items){
  ...
  return [...grouped.entries()].map(([section,rows])=>
    '<div class="gantt-checklist-section">'
      +'<div class="gantt-checklist-section-title">'+esc(section)+'</div>'
...
function renderGanttProjectChecklistSection(project){
  ...
  return '<div class="gantt-detail-pane gantt-checklist-pane">'
    +'<div class="gantt-detail-section gantt-detail-section--flush">'
      +'<div class="gantt-detail-section-head"><div><div class="gantt-panel-title">프로젝트 체크리스트</div><div class="gantt-detail-meta">반복 업무 확인 항목을 프로젝트별로 적용하고 완료율을 관리합니다.</div></div><div class="gantt-detail-head-actions"><button type="button" class="btn sm" onclick="openGanttChecklistTemplateModal(\''+projectId+'\')">체크리스트 적용</button>'+(state.checklist?'<button type="button" class="btn primary sm" onclick="saveGanttProjectChecklistItems(\''+projectId+'\')">저장</button>':'')+'</div></div>'
      +(meta.loading?'<div class="gantt-detail-empty">체크리스트를 불러오는 중...</div>'
        :meta.error?'<div class="gantt-detail-empty">'+esc(meta.error)+'</div>'
        :!state.checklist?renderGanttChecklistEmptyState(project)
        :'<div class="gantt-checklist-summary"><div><strong>'+summary.completion+'%</strong><span>완료율</span></div><div><strong>'+summary.requiredOpen+'</strong><span>필수 미완료</span></div><div><strong>'+summary.total+'</strong><span>전체 항목</span></div></div>'
          +'<div class="gantt-checklist-progress"><span style="width:'+Math.max(0,Math.min(100,summary.completion))+'%"></span></div>'
          +renderGanttChecklistItems(state.items||[])
          +'<div class="gantt-detail-inline-actions gantt-checklist-footer"><button type="button" class="btn primary sm" onclick="saveGanttProjectChecklistItems(\''+projectId+'\')">체크리스트 저장</button></div>')
    +'</div>'
  +'</div>';
}
  ...
  try{
    await Promise.all(rows.map(row=>{
      ...
      if(status==='completed'){
        ...
        body.completed_by=currentUser?.id||null;
      }else{
        body.completed_at=null;
        body.completed_by=null;
      }
      return api('PATCH','project_checklist_items?id=eq.'+id,body);
    }));
    const state=ganttProjectChecklistByProjectId[String(projectId)];
    if(state?.checklist?.id){
      await api('PATCH','project_checklists?id=eq.'+state.checklist.id,{updated_at:new Date().toISOString()}).catch(()=>null);
    }
    delete ganttProjectChecklistByProjectId[String(projectId)];
    await loadGanttProjectChecklist(projectId,true);
    if(typeof showToast==='function')showToast('체크리스트가 저장되었습니다.','success');
  }catch(error){
